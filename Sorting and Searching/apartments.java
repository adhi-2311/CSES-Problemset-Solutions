...
{  
    ...
    public static void process()throws IOException
    {
     ...
     sort(app,n);
     long size[]=larr(m);
     sort(size,m);//shuffle and sort to prevent TLE 
     int i=0,j=0;
     int ans=0;
     while(i<n && j<m)
     {
        if(app[i]<=size[j]+k && app[i]>=size[j]-k)//within range
            {ans++;i++;j++;}
        else if(app[i]>size[j]+k)//if greater than range,go for next size,as the next element would be greater than x+k
            j++;
       else
           i++;//if less than x-k,posssibility of finding an apartment within the range
     }
     pn(ans);
